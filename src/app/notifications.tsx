...
import { StyleSheet, Text, View, TouchableOpacity, SafeAreaView, FlatList, Platform, ActivityIndicator, StatusBar } from 'react-native';
...
export default function NotificationsScreen() {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      ...
      console.log('[Notifications] Response:', JSON.stringify(json).substring(0, 500));
  ...
  const handleMarkAllRead = async () => {
    ...
    } catch (e) { console.log('[Notifications] MarkAll error:', e); }
  };
  ...
  const handleReadNotification = async (id: number) => {
    ...
    } catch (e) { console.log('[Notifications] MarkRead error:', e); }
  };
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <LinearGradient colors={[COLORS.primary, '#0284C7']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.header}>
        <View style={styles.headerRow}>
          <TouchableOpacity onPress={() => router.canGoBack() ? router.back() : router.replace('/(tabs)/profile')} style={styles.backBtn} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <NotificationItem item={item} onMarkRead={handleReadNotification} />
          )}
